...
| DET-01 | P3 | Save | Loading a save repriced every academy scholar from his current ability: a lad whose ability had grown since his deal came back £50 a week dearer. The one place a reload changed the simulation | `scripts/qa/determinism.ts 60 777 migrate` | Reload is identity | Wages differ at step 1 | migrate ran `playerWage` unconditionally | Reprice only a wage outside the development band | `scripts/qa/determinism.ts` |
