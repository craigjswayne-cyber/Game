...
| Exploit resistance | 9 | The fuzzer now exists (`scripts/qa2/fuzz.ts`: 23 actions, any order, stale ids, bad amounts, six seeds) and found one gate (TREASURY-02); a 10 wants it in the suite on every push with more seeds |
...
| QA-GATE-03 | P3 | QA | The deep walkthrough could not have passed since the board's asking-twice sacking (v1.1.4) and the Shortlist heading rename: its showcase builder asked for a facility every seven weeks whatever the board said, so the 12-season manager was sacked and the walkthrough opened on a sacking modal; its Shortlist step waited for "Commissioned Search". Hidden by QA-GATE-01 | `npx vite-node scripts/deepsave.ts && node scripts/e2edeep.mjs` on 1.6.4 | A walkthrough of an employed manager's late-game screens | A sacking modal, then a timeout | Harness and builder drifted from the game | Builder stops at the first strike and fails if the manager is out; the locator reads the heading | `scripts/e2edeep.mjs` (run in `suite.sh all`) |
...
| TREASURY-02 | P3 | Treasury | `releaseToBudget` took any number: a NaN reached `budget +=` and poisoned the allowance, a negative amount moved money the wrong way. The screen never sends either; the engine took them | `scripts/qa2/fuzz.ts` (new): `releaseToBudget(g, NaN)` then `club.budget` is NaN | Refused | Budget NaN | No argument check | A NaN, infinite or non-positive amount is refused with its own message in six languages | `scripts/qa2/fuzz.ts`, six seeds, 1,500 actions each |
...
1. **The men's leagues play through the World Championship** (decided: they do, owner, 16 September). A 26-round Top 14 uses 26 of the 28 league weeks, so the overlay stays for league rounds only. The owner's one condition, that no semi-final or final of anything sits against the tournament, holds by construction and by the invariant: the men's tournament is weeks 1 to 12 and the earliest club knockout is week 34; the women's is weeks 1 to 6 and the earliest is week 28. `calinvariant.ts` states this check by name.
2. **The red card law** (RED-CARD-01). Decided: the permanent red stays. If World Rugby makes the twenty-minute replacement law in 2026 the decision can be revisited; nothing in the engine assumes either.
...
| Build and every browser harness again on aceb07f | `npm run build`, then each of the 56 `scripts/*.mjs` gates in the suite's browser list | PASS: build clean, 56 of 56 harnesses (the suite counts the build as its 57th line) | about 25 min |
...
| Deep walkthrough on a rebuilt 12-season showcase save | `npx vite-node scripts/deepsave.ts`, then `node scripts/e2edeep.mjs` | PASS: DEEP QA COMPLETE, no console errors, once the showcase builder stopped asking the board after a written no (its manager was being sacked for asking twice, so the walkthrough opened on a sacking modal) and its Shortlist step waited for the heading the screen has ("Scouting") | 53 s build, 30 s walkthrough |
