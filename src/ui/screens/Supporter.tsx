 ...
 */
/** A generic ownable row: price from the store, one buy button, every ending
 *  a sentence. The caller hands in TRANSLATED text (so the i18n probe sees
 *  every key at a real t() call site); the License keeps its own shelf below
 *  because its body copy points at the wizard. */
function OwnableShelf({ sku, title, sub, body, owned, thanks }: {
  sku: string; title: string; sub: string; body: string; owned: string; thanks: string
}) {
  ...
  const open = tillOpen()
  useEffect(() => { if (open) void skuPrice(sku).then(setPrice) }, [open, sku])
  if (!open) return null
  const isOwned = hasEntitlement(sku)
  ...
  const doBuy = async () => {
    ...
    setBusy(false)
    if (out === 'owned') claim()
    setMsg(out === 'owned' ? thanks
      : t(out === 'cancelled' ? 'supporter.cancelled'
  ...
  return (
    <>
      ...
      <div className="card" style={isOwned ? { borderLeft: '4px solid var(--gold)' } : undefined}>
        <div className="meta">{isOwned ? owned : body}</div>
        {!isOwned && (
